...
#define IN_PORT 9910
#define OUT_PORT 9921
#define DST_PORT 9922

int main(int argc, char *argv[]) {
	if(argc < 3) {
		printf("Usage: $0 dfe_top_ip dfe_bot_ip\n");
		return 1;
	}
	...
	max_udp_socket_t *dfe_socket = max_udp_create_socket_with_number(engine, "udpTopPort1", 1);
  ...
  max_udp_socket_t *dfe_socket_bot = max_udp_create_socket_with_number(engine, "udpBotPort1", 2);
  max_udp_bind(dfe_socket_bot, OUT_PORT);
